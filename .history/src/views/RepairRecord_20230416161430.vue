  ...
  methods: {
    ...
    edit(row) {
      ...
      this.$confirm("是否派遣维修？", "提示", {
        ...
        axios.put("http://localhost:8181/repair/update1/"+row.id).then(function(resp){
            if (resp.data.code == 0) {
                _this.$alert(_this.ruleForm.name + "已派遣人员维修", "", {
    ...
    del(row) {
      ...
      this.$confirm("是否完成维修？", "提示", {
        ...
        axios.put("http://localhost:8181/repair/update2/"+row.id).then(function(resp){
            if (resp.data.code == 0) {
                _this.$alert(_this.ruleForm.name + "已完成维修", "", {
                  confirmButtonText: "确定",
                  callback: (action) => {
                    _this.$router.push("/repairRecord");
                  },
                });
              }
        })
