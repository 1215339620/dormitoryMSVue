...
export default {
  data() {
    return {
      ...
      rules: {
        ...
        mId: [{ required: true, message: "请选择宿管姓名", trigger: "change" }],
      },
    };
  },
  methods: {
    submitForm(formName) {
      ...
      this.$refs[formName].validate((valid) => {
        if (valid) {
          axios
            .put("http://localhost:8181/building/update", _this.ruleForm)
            .then(function (resp) {
              if (resp.data.code == 0) {
  ...
  created() {
    const _this = this;
    axios
      .get("http://localhost:8181/dormitoryAdmin/list")
      .then(function (resp) {
        _this.dManagerList = resp.data.data;
      });
    axios
      .get("http://localhost:8181/building/findById/" + _this.$route.query.id)
      .then(function (resp) {
