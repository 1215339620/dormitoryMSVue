<template>
  ...
  >
    <el-form
      ...
      class="demo-ruleForm"
    >
      <el-form-item label="宿舍楼名称" prop="bName">
        <el-select v-model="ruleForm.bName" placeholder="请选择宿舍楼名称">
          <el-option
            v-for="item in bList"
            :label="item.name"
            :value="item.id"
      ...
      <el-form-item label="宿舍号" prop="name">
        <el-input v-model="ruleForm.name" placeholder="格式：楼号+宿舍号，1101"></el-input>
      </el-form-item>
...
export default {
  ...
  methods: {
    submitForm(formName) {
      const _this = this;
      var name=this.ruleForm.bName+this.ruleForm.name
