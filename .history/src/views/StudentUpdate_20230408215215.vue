<template>
  ...
  >
    <el-form
      ...
      class="demo-ruleForm"
    >
      <el-form-item label="学号" prop="username" readonly>
...
export default {
  ...
  methods: {
    submitForm(formName) {
      ...
      this.$refs[formName].validate((valid) => {
        if (valid) {
          axios
            .put("http://localhost:8181/student/update", _this.ruleForm)
            .then(function (resp) {
              if (resp.data.code == 0) {
  ...
  created() {
    ...
    axios
      .get("http://localhost:8181/student/findById/" + _this.$route.query.id)
      .then(function (resp) {
